#!/usr/bin/env python3
"""
YOLOv9 Object Detection with Per-Class Color Mapping
Each object class gets a unique, consistent color across all frames
"""
...
class ColorMapper:
    ...
    def get_class_info(self):
        """Get information about all seen classes and their colors"""
        info = {}
        for class_name in self.seen_classes:
            if class_name in self.class_colors:
                ...
                # Find the assigned random color
                info[class_name] = {
...
def main():
    """Main detection function with per-class colors"""
    ...
    print("Press 'q' to quit, 'c' to show class colors, 'r' to reset colors")
    ...
    try:
        while True:
            ...
            cv2.putText(frame, "Press 'q' to quit, 'c' for colors, 'r' to reset", (10, 70),
            ...
            if key == ord('q'):
                ...
                class_info = color_mapper.get_class_info()
                print("\n🎨 Class Colors:")
                print("-" * 40)
                for class_name, info in class_info.items():
                    color = info['color']
                    color_type = info['type']
                    print(f"{class_name:20} -> RGB{color} ({color_type})")
                print("-" * 40)
            elif key == ord('r'):
                # Reset color mapper
                color_mapper = ColorMapper()
                print("🔄 Color mapper reset!")
